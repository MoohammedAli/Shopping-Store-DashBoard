...
<?php
  if(isset($_POST['submit'])){
    ...
    $errors = array();
    if(empty($firstName) OR empty($lastName) OR empty($email) OR empty($password) OR empty($repeatedPassword)){
      array_push($errors,'There are empty fields');
    }
    if(!filter_var($email,FILTER_VALIDATE_EMAIL)){
      array_push($errors,'Email is not valid');
    }
    if(strlen($password) < 8){
      array_push($errors,'Password has to be more than 8 characters');
    }
    ...
    if(count($errors) > 0){
      ...
      echo "<div class='alert alert-danger'>User already exists</div>";
        ...
        if($stmtPrepare){
          ...
          echo "<div class='alert alert-warning'>Something is wrong with prepare statement</div>";
        }
      }
    }
  }
